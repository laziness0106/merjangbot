...
| `/랭킹 캐릭터` | 캐릭터 검색, 서버 선택, 전투력 전체·서버 순위와 전투력·매력·생활력·종합 점수 |

### 랭킹 조회 설치와 구조

현재 개발 환경의 헤드리스 Chromium 실조회는 Cloudflare HTTP 403으로 제한되었습니다. 브라우저 수집 코드와 카드·선택 메뉴는 구현했지만, 배포 환경의 실조회 성공은 확인되지 않았습니다. 접근이 제한되면 검색 링크를 대신 제공합니다. 서버에서 동일하게 제한되는 경우 사이트 운영자와 허용된 조회 경로를 협의해야 합니다.

랭킹은 Playwright Chromium으로 모비라이프 검색 페이지를 열고 렌더링된 표를 읽습니다. API 키는 필요 없습니다. Python 의존성 설치 후 **봇 실행 계정으로** `python -m playwright install chromium`을 실행하세요. Linux 서버에서는 `python -m playwright install --with-deps chromium`으로 브라우저의 시스템 의존성도 설치할 수 있습니다.

이름은 2~12자로 입력합니다. 결과가 여러 개면 명령어 실행자만 캐릭터를 선택할 수 있으며, 25개 단위로 이동합니다. 선택 메뉴는 5분 후 비활성화됩니다. 순위는 전투력 기준이고 사이트에서 읽은 값을 표시합니다. 원본 링크는 해당 이름의 검색 페이지로 연결됩니다.

브라우저는 첫 조회 때 시작하여 재사용하고 종료 시 정리합니다. 검색은 순차 실행하며 전체 대기는 최대 75초, 결과 캐시는 3분·최대 128개입니다. 캐시는 사이트 데이터의 갱신 시각을 의미하지 않습니다. 사이트 접속 제한이나 구조 변경으로 조회하지 못하면 실패 안내를 표시합니다.

| 파일 | 내용 |
| --- | --- |
| `sites/moblife/ranking.py` | 검색 URL, 랭킹 데이터 형식, 브라우저 조회·표 파싱·캐시 |
| `discord_bot/commands/ranking.py` | `/랭킹` 입력 검증과 조회 결과·오류 전송 |
| `discord_bot/presenters/ranking.py` | 캐릭터 정보와 전투력 순위 카드 |
| `discord_bot/views/ranking.py` | 캐릭터 선택, 페이지 이동, 조회자 확인, 원본 링크 |

`bootstrap.py`에서 조회 서비스를 생성하고 `discord_bot/bot.py`에서 명령어 등록과 브라우저 종료를 관리합니다.
